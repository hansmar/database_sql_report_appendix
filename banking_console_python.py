...
def view_family(conn):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT g.first_name || ' ' || g.last_name AS persónur,
    ...
    """)
    rows = cursor.fetchall()
    print_table(["Persónur", "Relatera til", "Samband"], rows)
    cursor.close()


def view_pending(conn):
    """Show all pending (unbooked) kassakladda entries."""
    print_header("Kassakladda - Óbókaðar transaktionir")
    cursor = conn.cursor()
    cursor.execute("""
    ...
    """)
    rows = cursor.fetchall()
    print_table(["TXN ID", "Slag", "Upphædd", "Frá", "Til", "Dato/Tíð", "Lýsing"], rows)
    cursor.close()


def view_journal(conn):
    """Show all journal entries."""
    print_header("Bókaðar transaktionir (Journal Entries)")
    cursor = conn.cursor()
    cursor.execute("""
    ...
    """)
    rows = cursor.fetchall()
    print_table(["ID", "Referansa", "Lýsing", "Dato", "Status"], rows)
    cursor.close()
...
def account_statement(conn):
    ...
    rows = cursor.fetchall()
    print_table(
        ["Dato/Tíð", "Slag", "Lýsing", "Frá", "Til", "Upphædd", "Status"],
        rows
    )
    print(f"  Núverandi saldo: {account[2]:,.2f} kr\n")
    cursor.close()
...
def deposit(conn):
    ...
    amount = get_number("  Upphædd: ")
    if amount is None or amount <= 0:
        print("  Upphædd má vera størri enn 0.")
        return

    description = get_input("  Lýsing: ", allow_empty=True) or "Innlegg"
    ...
    try:
        ...
        conn.commit()
        print(f"\n  Innlegg av {amount:,.2f} kr skrásett á kassakladdu. (TXN-{txn_id})")
        print(f"  Status: Pending - bóka fyri at gera galdandi.")
    except oracledb.Error as e:
        conn.rollback()
        print(f"\n  Feilur: {e}")
    finally:
...
def withdrawal(conn):
    ...
    print_header("Kassakladda - Úttøka (Withdrawal)")
    account_id = get_int("  Konto ID: ")
    if account_id is None:
        print("  Ógyldugt konto ID.")
        return
    ...
    description = get_input("  Lýsing: ", allow_empty=True) or "Úttøka"
    ...
    try:
        ...
        conn.commit()
        print(f"\n  Úttøka av {amount:,.2f} kr skrásett á kassakladdu. (TXN-{txn_id})")
        print(f"  Status: Pending - bóka fyri at gera galdandi.")
    except oracledb.Error as e:
        conn.rollback()
        print(f"\n  Feilur: {e}")
...
def transfer(conn):
    ...
    if from_id == to_id:
        print("  Frá og til konto kunnu ikki vera tað sama.")
        return
    ...
    description = get_input("  Lýsing: ", allow_empty=True) or "Flyting"
    ...
    try:
        ...
        conn.commit()
        print(f"\n  Flyting av {amount:,.2f} kr frá {from_id} til {to_id} skrásett á kassakladdu. (TXN-{txn_id})")
        print(f"  Status: Pending - bóka fyri at gera galdandi.")
    except oracledb.Error as e:
        conn.rollback()
        print(f"\n  Feilur: {e}")
...
def book_single(conn):
    ...
    print_header("Bóka eina transaktión")

    # Show pending first
    view_pending(conn)

    txn_id = get_int("  Transaction ID at bóka: ")
    if txn_id is None:
        print("  Ógyldugt transaction ID.")
        return
...
def book_all(conn):
    ...
    print_header("Bóka allar óbókaðar transaktionir")
    ...
    try:
        ...
        if count == 0:
            print("\n  Eingin óbókað transaktión at bóka.")
            return

        print(f"\n  {count} óbókaðar transaktionir funnir.")
        confirm = input("  Bóka allar? (j/n): ").strip().lower()

        if confirm == 'j':
            cursor.callproc("book_all_pending")
            conn.commit()
            print(f"\n  Allar {count} transaktionir eru bókaðar!")
        else:
            print("\n  Avlýst.")
    except oracledb.Error as e:
        conn.rollback()
...
def family_account_view(conn):
    ...
    person_id = get_int("  Títt person ID: ")
    if person_id is None:
        print("  Ógyldugt person ID.")
        return
...
def main_menu():
    ...
    while True:
        ...
        print("  7. Úttøka (Withdrawal)")
        print("  8. Flyting (Transfer)")
        print("  9. Vís óbókaðar transaktionir")
        print()
        print("  -- Bóking --")
        print("  10. Bóka eina transaktión")
        print("  11. Bóka allar óbókaðar")
        print("  12. Vís journal entries")
